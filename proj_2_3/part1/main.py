...
#softmax = run_softmax_on_MNIST(temp_parameter=1)
#print('softmax test_error=', softmax[0],"\n",
#      "softmax test_error_mod3", softmax[1])
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    # YOUR CODE HERE
    train_x, train_y, test_x, test_y = get_MNIST_data()
    theta, cost_function_history = softmax_regression(train_x, np.mod(train_y,3), temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_x, np.mod(test_y,3), theta, temp_parameter)
...
#print('softmax mod3 training test_error=', run_softmax_on_MNIST_mod3())
...
#print("softmax PCA training test error = ",run_softmax_on_PCA())
...
ten_components = 10
train_pca10 = project_onto_PC(train_x,pcs,ten_components,feature_means)
test_pca10 = project_onto_PC(test_x,pcs,ten_components,feature_means)
...
def run_softmax_on_PCA10(temp_parameter=1):
    """
    Trains Softmax regression on digit (mod 3) classifications.
...
#print("softmax PCA 10 training test error = ",run_softmax_on_PCA10())

# TODO: Train your softmax regression model using (train_cube, train_y)
#       and evaluate its accuracy on (test_cube, test_y).

plm.fit(train_pca10,train_y)
plm_pred = plm.predict(test_pca10)
#print('svm PLM test error = ',compute_test_error_svm(test_y,plm_pred))

rbf.fit(train_pca10,train_y)
rbf_pred = rbf.predict(test_pca10)
print('svm RBF test error = ',compute_test_error_svm(test_y,rbf_pred))